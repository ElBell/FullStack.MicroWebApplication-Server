...
You can also use the `.query()` method for HEAD requests. The following will produce the path `/blogUsers?name=joe@smith.com`.

      request
        .head('/blogUsers')
        .query({ name: 'joe@smith.com' })
        .then(function(res) {
...
In the _Node_ client Basic auth can be in the URL as "blogUser:pass":
...
Much like form fields in HTML, you can set field values with `.field(name, value)` and `.field({name: value})`. Suppose you want to upload a few images with your name and name, your request might look something like this:

     request
       .post('/upload')
       .field('blogUser[name]', 'Tobi')
       .field('blogUser[name]', 'tobi@learnboost.com')
