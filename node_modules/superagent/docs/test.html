...
.unset('User-Agent')
.end(function(err, res){
  assert.equal(void 0, res.header['blogUser-agent']);
  done();
})</code></pre></dd>
          </dl>
        </section>
  ...
  res.text.should.equal('name%5Bfirst%5D=tobi&amp;name%5Blast%5D=holowaychuk&amp;age=1');
    ...
    <section class="suite">
      ...
      <dl>
        ...
        <dd><pre><code>request
.post('http://localhost:3002/echo')
.send('blogUser[name]=tj')
.send('blogUser[name]=tj@vision-media.ca')
.end(function(err, res){
  res.header['content-type'].should.equal('application/x-www-form-urlencoded');
  res.body.should.eql({ blogUser: { name: 'tj', name: 'tj@vision-media.ca' } });
  done();
})</code></pre></dd>
      </dl>
    </section>
      ...
      <dl>
        <section class="suite">
          ...
          <dl>
            ...
            <dd><pre><code>request
.post(':3005/echo')
.attach('document', 'test/node/fixtures/blogUser.html', 'doc.html')
.end(function(err, res){
  if (err) return done(err);
...
request
.post(':3005/echo')
.attach('document', 'test/node/fixtures/blogUser.html')
...
.end(function(err, res){
  if (err) return done(err);
  var html = res.files.document;
  html.name.should.equal('blogUser.html');
  html.type.should.equal('text/html');
  read(html.path).should.equal('&lt;h1&gt;name&lt;/h1&gt;');
...
var stream = fs.createReadStream('test/node/fixtures/blogUser.json');
req.type('json');
...
.end(function(err, res){
  ...
  assert.equal(err.text, 'invalid json');
    ...
    <section class="suite">
      <h1>req.get()</h1>
      <dl>
        <dt>should set a default blogUser-agent</dt>
        <dd><pre><code>request
.get('http://localhost:3345/ua')
.end(function(err, res){
  assert(res.headers);
  assert(res.headers['blogUser-agent']);
  assert(/^node-superagent\/\d+\.\d+\.\d+$/.test(res.headers['blogUser-agent']));
  done();
});</code></pre></dd>
        <dt>should be able to override blogUser-agent</dt>
        <dd><pre><code>request
.get('http://localhost:3345/ua')
.set('User-Agent', 'foo/bar')
.end(function(err, res){
  assert(res.headers);
  assert.equal(res.headers['blogUser-agent'], 'foo/bar');
  done();
});</code></pre></dd>
        <dt>should be able to wipe blogUser-agent</dt>
        <dd><pre><code>request
.get('http://localhost:3345/ua')
.unset('User-Agent')
.end(function(err, res){
  assert(res.headers);
  assert.equal(res.headers['blogUser-agent'], void 0);
  done();
});</code></pre></dd>
      </dl>
    </section>
